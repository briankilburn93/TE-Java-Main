...
public class ProjectsCLI {
	...
	private void handleListAllDepartments() {
		...
		ResponseEntity<Department[]> responseEntity = apiCall.getForEntity(API_SERVER + "/departments",Department[].class);
	...
	private void handleListAllEmployees() {
		...
		// Call the APi to array of generic response - getForEntity instead of getForObject
		ResponseEntity<Employee[]> responseEntity = apiCall.getForEntity(API_SERVER + "/employees",Employee[].class);
