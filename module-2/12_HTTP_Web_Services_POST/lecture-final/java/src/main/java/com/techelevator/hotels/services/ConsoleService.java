...
// This is a class to handle interactions with a console (keyboard and screen)
  ...
  public int promptForHotel(Hotel[] hotels, String action) {
    ...
    // ensures no InputMisMatchException is thrown
    if (scanner.hasNextInt()) {           // if the user entered an integer
      menuSelection = scanner.nextInt();  //    go get it
      scanner.nextLine();                 //    clear scanner buffer 
    } else {                              // if integer not entered
      menuSelection = 999;                //    make selection=999
    }
    return menuSelection;
  }
