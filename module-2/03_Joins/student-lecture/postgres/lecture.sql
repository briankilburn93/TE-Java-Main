...
--ALTERNATE PATH OF ABOVE (causes duplicates)
SELECT p.*, c.first_name, c.last_name, r.return_date FROM payment AS p
JOIN customer AS c ON c.customer_id = p.customer_id
...
JOIN city ON city.id = country.capital; --Join makes it 232 because it will ignore the outside nulls
...
SELECT country.name, city.name FROM country
LEFT JOIN city ON city.id = country.capital; --left join makes it 239 because it adds nulls

--extra example
SELECT countrylanguage.language, country.name FROM countrylanguage -- FROM countrylanguage is the LEFT table
JOIN country ON country.code = countrylanguage.countrycode;        -- JOIN country is the RIGHT table
...
UNION
SELECT first_name, 'C'  AS SOURCE FROM actor
WHERE first_name --something
