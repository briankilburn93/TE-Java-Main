        ...
        while (menuSelection != 0) {
            ...
            try {
                if (menuSelection == 1) {
                    ...
                    // Be sure the credentials are exactly two Strings separated by a command
                    if( credentials.split(",").length == 2 ) {
                    	// Use the authentication service provided to login the user
                    	// This login method is expecting an Strings with "username,password"
                    	// This login method returns ResponsityEntity Map
                        ...
                        if (response.hasBody()) {
                            ...
                            // Assign the JWT to the variable used by HotelService to hold the JWT
                            hotelService.AUTH_TOKEN = token;
                            // Congratulate user on successfully
